...
module.exports = function (app, hexo) {
  ...
  app.use(hexo.config.root + 'txpadmin',  auth(new authStrategy(hexo)));
  app.use(hexo.config.root + 'txpadmin/login', function (req, res) {
      if (req.method === 'POST') {
          req.authenticate(['adminAuth'], function(error, done) {
              if (done) {
                  res.writeHead(302, { 'Location':  hexo.config.root + "txpadmin/" });
                  res.end();
              }
          });
      } else {
  ...
  app.use(hexo.config.root + 'txpadmin/', function (req, res, next) {
