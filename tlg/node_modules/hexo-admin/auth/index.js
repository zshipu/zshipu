...
module.exports = function (app, hexo) {
  ...
  app.use(hexo.config.root + 'admintlg', auth(new authStrategy(hexo)));
  app.use(hexo.config.root + 'admintlg/login', function (req, res) {
      if (req.method === 'POST') {
          req.authenticate(['adminAuth'], function(error, done) {
              if (done) {
                  res.writeHead(302, { 'Location':  hexo.config.root + "admintlg/" });
                  res.end();
              }
          });
      } else {
  ...
  app.use(hexo.config.root + 'admintlg/', function (req, res, next) {
