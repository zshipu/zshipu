...
module.exports = function (app, hexo) {
  ...
  app.use(hexo.config.root + 'adminm', auth(new authStrategy(hexo)));
  app.use(hexo.config.root + 'adminm/login', function (req, res) {
      if (req.method === 'POST') {
          req.authenticate(['adminAuth'], function(error, done) {
              if (done) {
                  res.writeHead(302, { 'Location':  hexo.config.root + "adminm/" });
                  res.end();
              }
          });
      } else {
  ...
  app.use(hexo.config.root + 'adminm/', function (req, res, next) {
